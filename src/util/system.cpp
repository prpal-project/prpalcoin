...
const char * const BITCOIN_CONF_FILENAME = "prpalcoin.conf";
...
#else
    const char* pszModule = "prpalcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Prpalcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Prpalcoin";
#else
    // Unix
    return pathRet / ".prpalcoin";
#endif
#endif
}
